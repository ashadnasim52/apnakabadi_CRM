...
import { Lead, Customer, StaffMember } from '../../types';
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <SearchSelect<StaffMember>
                        items={activeStaff}
                        onSelect={(s) => setFormData({...formData, assignedTo: s.id})}
                        onAddNew={(search) => setAddModal({ isOpen: true, type: 'staff', initialValue: search })}
                        labelField="name"
                        secondaryField="role"
                        placeholder="Select staff..."
                        idField="id"
                        defaultValue={activeStaff.find(s => s.id === formData.assignedTo)?.name}
                    />
                </div>
                <div>
      ...
      <UniversalAddModal 
        ...
        onSuccess={(id) => {
            if (addModal.type === 'customer') {
                ...
                if (newCustomer) handleCustomerSelect(newCustomer);
            } else if (addModal.type === 'staff') {
