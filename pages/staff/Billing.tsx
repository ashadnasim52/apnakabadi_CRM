...
import { BillItem, Bill, TransactionType, PaymentStatus, Customer, VehicleInfo, ScrapItem } from '../../types';
...
import { UniversalAddModal, EntityType } from '../../components/UniversalAddModal';
...
export const Billing = () => {
  ...
  const [addModal, setAddModal] = useState<{ isOpen: boolean, type: EntityType, initialValue: string }>({
    isOpen: false,
    type: 'customer',
    initialValue: ''
  });
  ...
  const isVehicleItem = !!selectedItemData && (selectedItemData.category === 'Vehicle' || /\b(vehicle|car|bike|truck|bus|rickshaw|auto|tractor)\b/i.test(selectedItemData.name) && !/battery/i.test(selectedItemData.name));
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 lg:space-y-6 pb-20 lg:pb-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-4 md:gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-4 lg:space-y-6">
            ...
            <div className="bg-white p-4 lg:p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                </div>                {/* Customer Search Dropdown */}
                <div className="mb-4">
                    <SearchSelect<{ id?: string, name: string, phone: string, address: string, type: 'Customer' | 'Lead' }>
                        items={allContacts}
                        onSelect={selectContact}
                        onAddNew={(search) => setAddModal({ isOpen: true, type: 'customer', initialValue: search })}
                        labelField="name"
                    ...
                    <div className="md:col-span-2 mt-2">
                        ...
                        <div className="flex space-x-2">
                            <button
                                ...
                                className={`flex-1 py-2 rounded-lg border-2 font-bold text-xs transition-all ${paymentStatus === 'Paid' ? 'bg-green-100 border-green-600 text-green-700' : 'bg-white border-slate-200 text-slate-400'}`}
                            >
                                PAID
                            </button>
                            <button
                                onClick={() => setPaymentStatus('Pending')}
                                className={`flex-1 py-2 rounded-lg border-2 font-bold text-xs transition-all ${paymentStatus === 'Pending' ? 'bg-red-100 border-red-600 text-red-700' : 'bg-white border-slate-200 text-slate-400'}`}
                            >
                                PENDING
                            </button>
                        </div>
                ...
                <div className="grid grid-cols-12 gap-3 items-end">
                    ...
                    <div className={`col-span-12 ${isOthers ? 'md:col-span-3' : (isVehicleItem ? 'md:col-span-6' : 'md:col-span-4')}`}>
                        ...
                        <SearchSelect<ScrapItem>
                            ...
                            onAddNew={(search) => setAddModal({ isOpen: true, type: 'item', initialValue: search })}
                            labelField="name"
      ...
      <UniversalAddModal 
        ...
        onSuccess={(id) => {
            if (addModal.type === 'customer') {
                const newCustomer = customers.find(c => c.id === id);
                if (newCustomer) selectContact({ ...newCustomer, type: 'Customer' });
            } else if (addModal.type === 'item') {
